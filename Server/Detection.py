...
def run_detection(video_path):
    """Process a video file frame‑by‑frame using the same pipeline
    that the live camera used.  Returns a dictionary containing any
    detected shots and the path for debugging.
    """
    shots = []
    # open video capture and get fps for timestamping
    ...
    while True:
        ...
        if not ret:
            break
        # maintain timestamp in seconds relative to start
        ts = frame_idx / fps
        frame_idx += 1
        ...
        # detect people with YOLO (class 0)
        ...
        for result in results:
            ...
            for box in result.boxes:
                ...
                if pose_results.pose_landmarks:
                    ...
                    # adjust landmarks back into coordinates of the full frame
                    for lm in landmarks:
